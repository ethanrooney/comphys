...
int main(int argc, char** argv)
{
  ...
  for(int i=0; i<nstep; ++i)
  {
    ...
    for(int j=0; j<NBODY; ++j)
    {
      if(yn[2*j+0] > L)
      {
        yn[2*j+0] = L - (yn[2*j+0]-L);
        yn[2*NBODY+2*j+0] *= -1;
        impulse[0] += -2*yn[2*NBODY+2*j+0]*mass;
      }
      if(yn[2*j+0] < 0)
      {
        yn[2*j+0] *= -1;
        yn[2*NBODY+2*j+0] *= -1;
        impulse[1] += 2*yn[2*NBODY+2*j+0]*mass;
      }
      if(yn[2*j+1] > L)
      {
         yn[2*j+1] = L - (yn[2*j+1] - L);
         yn[2*NBODY + 2*j+1] *= -1;
         impulse[2] += -2*yn[2*NBODY + 2*j+1]*mass;
      }
      if(yn[2*j+1] < 0)
      {
         yn[2*j+1] *= -1;
         yn[2*NBODY+2*j+1] *= -1;
         impulse[3] += 2*yn[2*NBODY+2*j+1]*mass;
      }
    }
    ...
    if((i+1)%nskip == 0)
    {
      ...
      for(int k=0; k<4; ++k) printf("%20.15e ", impulse[k]);
      printf("\n");
    }
